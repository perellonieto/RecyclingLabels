...
unique_file = 'Example_17_{}_a{}_r{:03.0f}_{}'.format(dataset_name, alpha*100, random_state,
...
files_list = glob.glob("./Example_17*summary.csv")
...
for name, df_ in df_grouped:
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(os.path.join('Example_17_{}_a{:03.0f}.svg'.format(dataset_name,
                                                         float(name[0])*100)))
