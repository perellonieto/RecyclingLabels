...
def save_model(path, model, process_id):
    filename_json = os.path.join(path, "model_{}.json".format(process_id))
    filename_h5 = os.path.join(path, "model_{}.h5".format(process_id))
    print("Saving model to {}".format(filename_h5))
...
def train_weak_Mproper_test_results(parameters):
    ...
    history = classifier.fit(X_t, V_t, X_y_t=X_y_t, Y_y_t=Y_y_t,
                             **fit_arguments)
    # 4. Evaluate the model in the validation set with true labels
...
def train_weak_fully_weak_test_results(parameters):
    ...
    # Add validation results during training
    fit_arguments['validation_data'] = (X_y_v, Y_y_v)
    history = classifier.fit(X_z_t, Z_z_t, X_y_t=X_y_t, Y_y_t=Y_y_t,
                             **fit_arguments)
    # 2. Evaluate the model in the validation set with true labels
    y_pred = classifier.predict(X_y_v, verbose=verbose)
    # Compute the confusion matrix
...
def analyse_weak_labels(X_z, Z_z, z_z, X_y, Z_y, z_y, Y_y, y_y, classes,
    ...
    for i in range(n_iterations):
        ...
        for train, valid in splits:
            ...
            n_extra.add_entry(row={'pid': process_id,
                                   'Counter(z_y_t)': Counter(z_y_s[train])})
            n_extra.add_entry(row={'pid': process_id,
                                   'z_y_t': z_y_s[train][:5]})
            n_extra.add_entry(row={'pid': process_id,
            ...
            n_extra.add_entry(row={'pid': process_id,
                                   'Counter(y_y_v)': Counter(y_y_s[valid])})
            n_extra.add_entry(row={'pid': process_id,
                                   'y_y_v': y_y_s[valid][:5]})
            n_extra.add_entry(row={'pid': process_id,
                                   'Y_y_v': "\n{}".format(Y_y_s[valid][:5])})
            n_extra.add_entry(row={'pid': process_id,
                                   'Counter(z_y_v)': Counter(z_y_s[valid])})
            n_extra.add_entry(row={'pid': process_id,
                                   'z_y_v': z_y_s[valid][:5]})
            n_extra.add_entry(row={'pid': process_id,
...
def train_and_test_weak_labels(X_z, Z_z, z_z, X_y, Z_y, z_y, Y_y, y_y, X_te,
                               ...
                               batch_size=100, rho=0.9, epsilon=None):
    """ Trains a Feed-fordward neural network using cross-validation
